...
module LogisticRegressionPar where
...
gradDesc' xy w α it
  ...
  where
    ...
    partial (xi, yi) = ( yi - (sigmoid $ dotprod w xi) ) *. xi    
    convergiu = (it==0)
    n         = sum $ map length' xy
...
accuracy :: ChunksOf [(Vector Double, Double)] -> Vector Double -> Double
accuracy xy w = (length' correct) / n
  where
  
    correct = filter (id) $ concat 
            $ (map mapper xy `using` parList rdeepseq)
    mapper xyi = filter (id) $ map (\(x,y) -> fhat x == y) xyi
